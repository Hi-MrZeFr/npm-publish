let mr = {}; //初始化函数
mr.showRightMenu = function(isTrue, x=0, y=0){
...
//复制选中文字
mr.copySelect = function(){
    document.execCommand('Copy',false,null);
}

//回到顶部
mr.scrollToTop = function(){
...
if(! (navigator.userAgent.match(/(phone|pad|pod|iPhone|iPod|ios|iPad|Android|Mobile|BlackBerry|IEMobile|MQQBrowser|JUC|Fennec|wOSBrowser|BrowserNG|WebOS|Symbian|Windows Phone)/i))){
    window.oncontextmenu = function(event){
        ...
        if (localStorage.getItem("mr_switch")==='off') return true
        ...
        mr.showRightMenu(true, pageY, pageX);
        return false;
    };

    window.addEventListener('click',function(){mr.showRightMenu(false);});
}
//打开Tidio
mr.openTidio = function(){
    window.tidioChatApi.show();window.tidioChatApi.open();
    btf.snackbarShow("打开Tidio成功!");    
    }
//点击及加载
$('#menu-home').on('click',function(){window.location.href = window.location.origin;});
$('#menu-archives').on('click',function(){window.location = "/archives/";});
$('#menu-tags').on('click',function(){window.location = "/tags/";});
$('#menu-categories').on('click',function(){window.location = "/categories/";});
...
$('#menu-read').on('click',function(){mr.switchReadMode();});
//copyurl
mr.copyUrl = function(id) {
...
//关闭右键菜单
function mrmenuOff(){
  localStorage.setItem("mr_switch", "off");
  btf.snackbarShow("已关闭右键菜单!");
}
//控制右键菜单开关
function mr_switch(){
  var mr_switch = localStorage.getItem("mr_switch");
  if (mr_switch === "off"){
    localStorage.setItem("mr_switch", "on");
    btf.snackbarShow("已开启右键菜单!");
    mr.showRightMenu(true);
  }else { 
    mrmenuOff()
  }
}
